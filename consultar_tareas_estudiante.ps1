...
try {
    ...
    foreach ($estudiante in $loginResponse.estudiantes) {
        ...
        try {
            $tareasResponse = Invoke-RestMethod -Uri "$baseUrl/api/movil/tareas?estudianteId=$($estudiante.id)" -Method GET -Headers $headers -UseBasicParsing
            ...
            if ($tareasResponse.data -and $tareasResponse.data.Count -gt 0) {
                ...
                Write-Host "`n📊 RESUMEN DE TAREAS:" -ForegroundColor Magenta
                ...
                Write-Host "`n📚 DETALLES DE TAREAS:" -ForegroundColor Magenta
                foreach ($tarea in $tareasResponse.data) {
                    $estadoIcon = switch ($tarea.estado) {
                        "pendiente" { "📝" }
                        "completada" { "✅" }
                        "vencida" { "⏰" }
                        "calificada" { "🎯" }
                    ...
                    Write-Host "$estadoIcon [$($tarea.estado.ToUpper())] $($tarea.titulo)"
                    Write-Host "   📅 Entrega: $($tarea.fechaEntrega)"
                    if ($tarea.descripcion) {
                        Write-Host "   📝 $($tarea.descripcion.Substring(0, [Math]::Min(80, $tarea.descripcion.Length)))..."
                    }
                    Write-Host ""
                }
