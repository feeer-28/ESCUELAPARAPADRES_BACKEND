...
# Paso 1: Despertar servidor (puede tardar 30-60 segundos)
...
try {
    # Primera petición con timeout largo para despertar
    ...
    if ($response.success) {
        Write-Host "✅ SERVIDOR DESPIERTO - LOGIN EXITOSO!" -ForegroundColor Green
        
        # Paso 2: Probar FCM Token inmediatamente
    ...
    if ($_.Exception.Message.Contains("timeout") -or $_.Exception.Message.Contains("tiempo")) {
        Write-Host "⚠️  El servidor está muy lento o en suspensión" -ForegroundColor Yellow
        Write-Host "   Esto es normal en Render.com - intenta de nuevo en 1 minuto" -ForegroundColor Yellow
    }
}
